...
if [ $# -lt 3 ]; then
	echo "usage: $0"
        echo "      dest_cloud_name:vendor_dir api_doc_dir"
...
code_dir=$code_home_dir/vendor/github.com/huawei-clouds/golangsdk/openstack/$(echo $1 | awk -F ':' '{print $2}')
mkdir -p $code_dir
...
func Get(c *golangsdk.ServiceClient${service_client_version}, id string) (r GetResult) {
	_, r.Err = c.Get(resourceURL(c, id), &r.Body, nil)
	return
}

func Delete(c *golangsdk.ServiceClient${service_client_version}, id string) (r DeleteResult) {
	reqOpt := &golangsdk.RequestOpts{OkCodes: []int{${delete_success_codes}}}
...
fs=("create_resp.docx" "update_resp.docx" "get.docx")
for fd in ${fs[@]}
do
    fd=$api_doc_dir/$fd
    if [ -f $fd ]; then
